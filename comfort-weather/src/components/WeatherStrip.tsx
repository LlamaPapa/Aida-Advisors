...
function tempDisplay(tempF: number, units: Units): string {
  if (units === "C") {
...
export default function WeatherStrip({ hourly, units }: Props) {
  return (
      ...
      {hourly.map((h) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-sm font-medium">{tempDisplay(h.temp, units)}</span>
            {precip && (
